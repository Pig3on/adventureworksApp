...
function ItemList({ match }) {
    ...
    const onDataSaveHandle = useCallback(() => {
            ...
            <div style={{ display: "flex", alignItems: 'center', justifyContent: 'center', flexDirection: 'column' }}>
                ...
                <div>
                    ...
                    <Modal
                        ...
                        onCancel={closeForm}
                    >
                        <AddItem billId={match.params.id} onDataSave={onDataSaveHandle} />
                    </Modal>
                </div>
            </div>
            <PageableList items={items} renderItem={(item) => {
                return (
                    <ItemListItem
                        isLoggedin={isLoggedIn}
                        onSecondPress={() => { executeFunctionAfter(removeItem, [item.Id], 10) }}
                        item={item}
                    />)
            }}
                itemsPerPage={10} />
